...
def lanzar_recompensa(recompensa: dict) -> Optional[subprocess.Popen]:
    ...
    if tipo == "url":
        ...
        comando = f"sudo -u {USUARIO_LINUX} DISPLAY=:0 google-chrome --kiosk --app={url}"
...
def cerrar_recompensa():
    ...
    if proceso_recompensa:
        ...
        os.system("pkill -f 'google-chrome --kiosk --app' > /dev/null 2>&1")
...
def cerrar_kiosco_home():
    ...
    if proceso_kiosco_home:
        os.system(f"pkill -u {USUARIO_LINUX} google-chrome > /dev/null 2>&1")
